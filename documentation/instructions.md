...
You can change these settings by going to Applications Menu (raspberry icon on taskbar) > Preferences > Raspberry Pi Configuration. Once the configuration window opens, go to the Interfaces tab and enable camera, I2C, Serial Port, and Serial Console. Once you enable these, the RPi will ask to reboot.
...
After you have soldered the breakaway headers to the RPi Zero W, attach the PiRTC with the coin cell installed. The directions outlined below are specific to the DS3231 PiRTC and also available in the [documentation/adafruit folder](https://github.com/IanTBlack/corona_cam/blob/master/documentation/adafruit/pi_rtc_adafruit_guide.pdf).

First, make sure the Pi is set to the timezone that you want. To make it simple, I usually set the locale to UK or the timezone to UTC.
...
```
On the line below that one, type:
```bash
...
## 7) Camera Side Setup
Install the 3D printed camera mount into the 3" Blue Robotics Flange using 10mm long, 3mm hex bolts. Then install 8mm M2.5 standoffs for the camera and 6mm M2.5 standoffs for the Pi. Note that M2.5mm hex standoffs can be tightened with a 5mm hex socket.
...
Through the opposite side, thread the NeoPixel wires through the slots. The NeoPixel should sit freely within the grooves in the 3D print.
...
[The pinouts for USB hardware can be found on Wikipedia.](https://en.wikipedia.org/wiki/USB_hardware)
Since this cable will under go some strain when removing and attaching the battery pack, I recommend using the [Lineman's Splice](https://www.youtube.com/watch?v=O-ymw7d_nYo)
...
You can confirm that the camera produced a video by navigating to the h264 folder. The file should be titled 'corona_cam_YYYY-MM-DD_HHMM.h264'.
...
Utilizing this raspistill command is useful when tuning the camera.  Install the acrylic end cap over the camera along with the 3D printed setup tool. Adjust the camera lens until it is focused on the 3D print. The distance between the camera lens and the print is approximately 4cm. If the camera lens protrudes further than the end cap, you will need to break down the camera and adjust the length of the M2.5 standoffs that are used to hold the camera in place. Once focused, the viewing area of the camera is approximately 2cm x 2cm.
...
The second script is the main operating code for the camera. You can set the frequency that the camera records via Cron. This can be done as an interval (i.e. every 60 minutes), or you can set it up to record at a specific time (i.e every hour at two minutes past the hour). Remember, this just modifies how often the camera records. If you want to change the length of time that the camera records, you'll need to change the record_time value in the Python script.

Note that each script will need its own line in crontab.

To access crontab, issue the following command in terminal:
...
This will run the script every time the Pi reboots. Next we will want to run the operating Python script.
If you want to run the script every 15 minutes (independent of the clock time), enter the following on a new line:
...
This becomes particularly handy if you modify the corona_cam script and make copies that operate for different time lengths, take pictures, have different camera resolutions, different LED colors, etc. You could have two different scripts run at different times. [Crontab Guru](https://crontab.guru/#*_*_*_*_*) is a handy tool for determining crontab scheduling.

Once you have set up the lines in crontab file, write out the changes with CTRL + O, and then exit with CTRL + X.

To check if your changes were made, issue the command:
